...
mazePlot <- function(feedersData, wantedFeeder = -1){
  feeders <- apply(feedersData,1, function (feeder) {
    ...
    l <- .25
    zone <- data.frame(x=c(x+l,x+l,x-l,x-l,x+l),y=c(y+l, y-l, y-l,y+l,y+l))
    zone_line <- geom_path(data=zone, aes(x=x,y=y), color="orange")
    list(p, zone_line)
  })
...
ratPathPlot <- function(pathData, p){
  ...
  p + geom_segment(data=pathSegs[c('x','y','nX','nY','random')], aes(x,y,xend=nX,yend=nY,color = random), cex=.2) + scale_color_manual(values=c(true="red", false="blue")) 
}
