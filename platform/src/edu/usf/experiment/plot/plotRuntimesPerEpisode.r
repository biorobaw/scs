...
plotArrival <- function(pathData, plotName){
  summarizedRunTimes <- ddply(pathData, .(trial, repetition, group), summarise, sdRT = sd(runtime)/sqrt(length(runtime)), mRT = mean(runtime))
  summarizedRunTimes <- ddply(summarizedRunTimes, .(trial, group), summarise, repetition=repetition, mRT=mRT, sdRT=sdRT, runmedian = runmed(mRT, 31))
  p <- ggplot(summarizedRunTimes, aes(x=repetition, y = mRT, color = group)) 
  ...
  ggsave(plot=p,filename=paste("runtimesPerEpisode.pdf", sep=''), width=10, height=10)
}
...
plotArrival(runtimes, plotName="runtimes")
