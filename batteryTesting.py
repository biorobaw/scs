...
if __name__=='__main__':
  ...
  def processFile(fileName):
    global columnNames,columnTypes,columnValues,combinationType,obligatoryColumns,optionalArguments,readingColumn,possibleColumns
    f = open(fileName)
    i=0
    for l in f:
#      print 'line',l
#      print 'slit',l.split()
      ...
      if len(line)==0 or line[0][0]=='#':
        ...
        possibleColumns = possibleColumns + line[1:]
...
#          print 'is obligatory'
  ...
  def parseError(col):
    global columnNames
    print 'ERROR: couldn\'t parse calculated column ',columnNames[col]
  ...
  def createOperation(tokens,col):
    ...
    if tokens[nextToken] == 'lenList':
      ...
      def calculator(table,col,row):
        table[col][row] = row
      nextToken +=1
    elif tokens[nextToken] == 'concat':
      def calculator(table,col,row):
        table[col][row] = ""
        for i in range(nextToken+1,len(tokens)):
          if(tokens[i] in columnNames):
            colId = order[columnNames.index(tokens[i])]
            table[col][row] = table[col][row] + resultTable[colId][row]
          else:
            table[col][row] = table[col][row] + tokens[i][1:-1]
    else:
      parseError(col)
    return calculator
