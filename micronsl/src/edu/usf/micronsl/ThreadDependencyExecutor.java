...
public class ThreadDependencyExecutor extends ThreadPoolExecutor {
	...
	public ThreadDependencyExecutor(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit,
		...
		for (DependencyRunnable dr : tasks) {
			...
			for (DependencyRunnable dep : dr.getPreReqs()) {
				if (!byDependOn.containsKey(dep))
					byDependOn.put(dep, new LinkedHashSet<DependencyRunnable>());
			...
			if (!byNumDependencies.containsKey(numDeps))
				byNumDependencies.put(numDeps, new LinkedHashSet<DependencyRunnable>());
	...
	protected synchronized void afterExecute(Runnable r, Throwable t) {
		...
		if (Debug.printSchedulling)
			System.out.println("Completed a task: " + ((Module) completed).getName());
		...
		if (byDependOn.containsKey(completed))
			for (DependencyRunnable antiDep : byDependOn.get(completed)) {
				...
				if (numDeps == 0) {
					if (Debug.printSchedulling)
						System.out.println("Executing " + ((Module) antiDep).getName());
					execute(antiDep);
				}
			}
